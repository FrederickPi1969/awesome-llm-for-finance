#!/usr/bin/env python3
"""Third focused expansion for institutional trading and investment papers."""
...
DEFAULT_OUTPUT_ANCHORS = ROOT / "data" / "processed" / "institutional_trading_round3_anchor_candidates.csv"
DEFAULT_OUTPUT_EDGES = ROOT / "data" / "raw" / "institutional_trading_round3_edges.csv"
DEFAULT_OUTPUT_MANIFEST = ROOT / "data" / "raw" / "institutional_trading_round3_manifest.csv"

ANCHOR_STATUSES = {
    "institutional_trading_round2_promoted",
}
...
def anchor_rows(curated_rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for row in curated_rows:
        ...
        if row.get("list_status") not in ANCHOR_STATUSES:
            continue
...
def anchor_candidate_row(row: dict[str, str]) -> dict[str, str]:
    return {
        ...
        "matched_seed_ids": "INSTITUTIONAL-TRADING-ROUND3-ANCHOR",
        "matched_seed_titles": row.get("title", ""),
        "finance_terms": "options; derivatives; alpha mining; factor discovery; order book; market simulation; venture capital",
...
def source_prefix(row: dict[str, str]) -> dict[str, str]:
    return {
        "source_round": "institutional_trading_round3",
